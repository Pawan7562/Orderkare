          ...
          {ads.map((ad) => (
            ...
            >
              ...
              <div className="space-y-4">
                ...
                {/* Banner Mini Preview - Clean Light Theme */}
                <div className="rounded-2xl border border-slate-200/80 bg-white overflow-hidden shadow-xs">
                  <div className="relative aspect-[21/9] bg-slate-50 overflow-hidden">
                    <img src={ad.imageUrl} alt={ad.title} className="w-full h-full object-cover" />
                    <div className="absolute top-2 left-2 flex items-center gap-1.5">
                      <span className="text-[9px] font-extrabold uppercase bg-white/95 text-orange-600 border border-orange-200 px-2 py-0.5 rounded-full shadow-2xs backdrop-blur-md">
                        {ad.badge}
                      </span>
                    </div>
                  </div>
                  <div className="p-3 bg-white space-y-1 text-left">
                    <h4 className="text-xs font-black text-slate-900 leading-tight truncate">{ad.title}</h4>
                    <p className="text-[11px] text-slate-500 line-clamp-1">{ad.description}</p>
                    <div className="pt-1 flex items-center justify-between">
                      <span className="inline-flex items-center gap-1 text-[11px] font-black text-orange-600 font-mono">
                        <Percent className="w-3 h-3 stroke-[3]" />
                        <span>{ad.discountText}</span>
                      </span>
                      {ad.promoCode && (
                        <span className="bg-orange-50 text-orange-700 border border-orange-200 px-2 py-0.5 rounded-md font-mono font-bold text-[10px]">
                          {ad.promoCode}
                        </span>
                      )}
                    </div>
                ...
                <div className="space-y-2">
                  ...
                  {imageUploadMode === 'url' && (
                    <div className="space-y-2">
                      ...
                      {form.imageUrl ? (
                        <div className="h-16 rounded-xl overflow-hidden border border-slate-200 bg-slate-50 relative">
